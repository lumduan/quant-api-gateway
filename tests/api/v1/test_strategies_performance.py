"""Tests for ``GET /api/v1/strategies/{strategy_id}/performance``."""
...
def _make_cached_strategy_perf() -> StrategyPerformanceResponse:
...
async def test_cache_hit_returns_cached_response(
    ...
    cached = _make_cached_strategy_perf()
    ...
    response = await async_client.get("/api/v1/strategies/csm-set-01/performance")

    assert response.status_code == 200
    body = response.json()
    assert body["strategy_id"] == "csm-set-01"
    assert float(body["daily_pnl"]) == 1000.0
...
async def test_unknown_strategy_returns_404(
    ...
    response = await async_client.get("/api/v1/strategies/nonexistent/performance")

    assert response.status_code == 404
    assert "nonexistent" in response.json()["detail"]


async def test_inactive_strategy_returns_404(
    async_client: AsyncClient,
    patch_lifespan_deps: None,
    monkeypatch: pytest.MonkeyPatch,
    mock_pool: Any,
) -> None:
    # Use a registry where the only strategy is inactive
    from src.schemas.registry import StrategyConfig

    inactive_registry = StrategyRegistry(
        strategies=[
            StrategyConfig(
                id="csm-set-01",
    ...
    monkeypatch.setattr("src.api.v1.performance.get_registry", lambda: inactive_registry)
...
async def test_db_failure_returns_500(
    ...
    monkeypatch.setattr("src.api.v1.performance.compute_strategy_performance", mock_compute)

    response = await async_client.get("/api/v1/strategies/csm-set-01/performance")

    assert response.status_code == 500
    assert "Failed to compute performance for strategy" in response.json()["detail"]
